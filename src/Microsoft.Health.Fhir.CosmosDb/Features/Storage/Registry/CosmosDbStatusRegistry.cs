...
namespace Microsoft.Health.Fhir.CosmosDb.Features.Storage.Registry
{
    public class CosmosDbStatusRegistry : ISearchParameterRegistry
    {
            ...
            do
            {
                ...
                parameterStatus.AddRange(results
                    .Select(x => new ResourceSearchParameterStatus
                    {
                        ...
                        IsPartiallySupported = x.IsPartiallySupported,
                    }));
            }
        ...
        public async Task UpdateStatuses(IEnumerable<ResourceSearchParameterStatus> statuses)
        {
            ...
            await UpsertStatuses(clientScope.Value, statuses);
        }

        internal async Task UpsertStatuses(IDocumentClient client, IEnumerable<ResourceSearchParameterStatus> statuses)
        {
            EnsureArg.IsNotNull(client, nameof(client));
            EnsureArg.IsNotNull(statuses, nameof(statuses));

            foreach (var status in statuses
                .Select(x => new SearchParameterStatusWrapper
                {
                    Uri = x.Uri,
                    Status = x.Status,
                    LastUpdated = x.LastUpdated,
            ...
            {
                await client.UpsertDocumentAsync(CollectionUri, status);
            }
        }
    }
}
